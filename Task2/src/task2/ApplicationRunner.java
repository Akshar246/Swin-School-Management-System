...
public class ApplicationRunner {
    ...
    private static List<SwimStudent> initializeStudents() {
        ...
        Instructor instructor2 = new Instructor("Ms. Kate wisely");
        ...
        SwimStudent student1 = new SwimStudent("Steve Rogers", "Novice");
        student1.lesson = lessonNovice;
        student1.addQualification(new Qualification("Distance Swim", "5m", instructor1));

        SwimStudent student2 = new SwimStudent("Kate Winslet", "Improver");
        ...
        SwimStudent student3 = new SwimStudent("Johnny Depp", "Advanced");
    ...
    private static void displayLessonDetails(String day, String time, String level) {
        for (SwimStudent student : students) {
            if (student.lesson != null && student.lesson.day.equals(day) && student.lesson.time.equals(time)
                    && student.level.equals(level)) {
                ...
                long count = students.stream().filter(s -> s.lesson != null && s.lesson.day.equals(day)
                        && s.lesson.time.equals(time) && s.level.equals(level)).count();
                System.out.println("Students in this class:");
                students.stream().filter(s -> s.lesson != null && s.lesson.day.equals(day) && s.lesson.time.equals(time)
                        && s.level.equals(level)).forEach(s -> System.out.println("- " + s.name));
    ...
    private static void displayInstructorSchedule(Instructor instructor) {
        ...
        for (SwimStudent student : students) {
            if (student.lesson != null && student.lesson.instructor.equals(instructor)) {
                ...
                System.out.println("- Day: " + student.lesson.day + ", Time: " + student.lesson.time + ", Level: "
                        + student.level);
                System.out.println("  Students:");
                students.stream()
                        .filter(s -> s.lesson != null && s.lesson.instructor.equals(instructor)
                                && s.lesson.day.equals(student.lesson.day) && s.lesson.time.equals(student.lesson.time))
    ...
    private static void addNewSwimStudent() {
        ...
        boolean availableClassFound = false;
        for (SwimLesson lesson : noviceLessons) {
            ...
            if (enrolledStudents < 4) {
                System.out.println("- Day: " + lesson.day + ", Time: " + lesson.time +
                        ...
                        ", Spaces available: " + (4 - enrolledStudents));
                availableClassFound = true;
            }
        }

        if (!availableClassFound) {
            System.out.println("No available novice classes. Adding " + studentName + " to the waiting list.");
            ...
            System.out.println(
                    "Enter the day and time of the class you want to add the student to (e.g., Monday 17:00), or type 'wait' to add to the waiting list:");
            String selection = scanner.nextLine();

            if ("wait".equalsIgnoreCase(selection)) {
                SwimStudent newStudent = new SwimStudent(studentName, "Novice");
                students.add(newStudent);
                System.out.println(studentName + " has been successfully added to the waiting list.");
            } else {
                boolean added = false;
                for (SwimLesson lesson : noviceLessons) {
                ...
                if (!added) {
                    System.out.println(
                            "Invalid selection or class is full. Adding " + studentName + " to the waiting list.");
                    SwimStudent newStudent = new SwimStudent(studentName, "Novice");
                    students.add(newStudent);
                    System.out.println(studentName + " has been added to the waiting list.");
                }
    ...
    private static void awardSwimQualification() {
        ...
        List<String> qualifications = Arrays.asList("5m", "10m", "20m", "50m", "100m", "200m", "400m", "800m",
                "Personal Survival Bronze", "Personal Survival Silver", "Personal Survival Gold");
        ...
        Qualification qualification = new Qualification(
                qualAwarded.contains("Survival") ? "Personal Survival" : "Distance Swim", qualAwarded, instructor);
    ...
    // Example method call, your application logic here
        ...
        while (true) {
            ...
            if (scanner.hasNextInt()) {
                ...
                if (choice >= 1 && choice <= waitingList.size()) {
                    selectedStudent = waitingList.get(choice - 1);
                    break;
                }
            } else {
                scanner.nextLine(); // clear the invalid input
            }
            System.out.println("Invalid selection. Please enter a number corresponding to the student.");
        }

        // Now, handle class selection
        System.out.println("Available Novice Classes:");
        // Your existing logic to display classes

        while (true) {
            System.out.println(
                    "Enter the day and time of the class to add the student to (e.g., Monday 17:00), or type 'none' to cancel:");
            ...
            // Your existing logic to find and add a student to a class

            if (classFound) {
                break; // Exit the loop if class is found and student is added
            } else {
                System.out.println("No available class found or invalid selection. Please enter a valid day and time.");
            }
        }
    }
}

// Assuming SwimStudent and SwimLesson classes are defined elsewhere in your
// code
